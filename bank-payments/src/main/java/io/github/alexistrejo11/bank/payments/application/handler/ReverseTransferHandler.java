...
public class ReverseTransferHandler {
	...
	@Transactional
	public Result<TransferResponse> handle(ReverseTransferCommand command) {
		UserId userId = command.userId();
		UUID idempotencyKey = command.idempotencyKey();
		UUID originalTransferId = command.originalTransferId();
		...
		if (source == null || !source.active()) {
			return persistFailureReverse(userId, idempotencyKey, original, "SOURCE_INVALID",
					"Cannot load source account for reversal");
		}
		AccountLedgerInfo target = accountLedgerInfoPort.find(AccountId.of(newTarget)).orElse(null);
		if (target == null || !target.active()) {
			return persistFailureReverse(userId, idempotencyKey, original, "TARGET_INVALID",
					"Cannot load target account for reversal");
		}
		...
		if (source.ledgerBalance().compareTo(amt) < 0) {
			return persistFailureReverse(userId, idempotencyKey, original, "INSUFFICIENT_FUNDS",
