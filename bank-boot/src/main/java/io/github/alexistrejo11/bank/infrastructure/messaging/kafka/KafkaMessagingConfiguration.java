...
public class KafkaMessagingConfiguration {
	...
	@Bean
	public ProducerFactory<String, BankDomainEvent> bankDomainEventProducerFactory(
			@Value("${spring.kafka.bootstrap-servers}") String bootstrapServers) {
		Map<String, Object> props = new HashMap<>();
		props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
		props.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
		props.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class);
		JsonSerializer<BankDomainEvent> jsonSerializer = new JsonSerializer<>();
		return new DefaultKafkaProducerFactory<>(props, new StringSerializer(), jsonSerializer);
	}

	@Bean("bankDomainEventKafkaTemplate")
	public KafkaTemplate<String, BankDomainEvent> bankDomainEventKafkaTemplate(
			ProducerFactory<String, BankDomainEvent> bankDomainEventProducerFactory) {
		return new KafkaTemplate<>(bankDomainEventProducerFactory);
	}
	...
	public ConcurrentKafkaListenerContainerFactory<String, BankDomainEvent> bankDomainEventKafkaListenerContainerFactory(
			...
			@Qualifier("bankDomainEventKafkaTemplate") KafkaTemplate<String, ?> kafkaTemplate) {
