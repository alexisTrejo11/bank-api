package io.github.alexistrejo11.bank.shared.shared_kernel.event;

import io.github.alexistrejo11.bank.shared.shared_kernel.ids.AccountId;
import io.github.alexistrejo11.bank.shared.shared_kernel.ids.LoanId;
import java.math.BigDecimal;
import java.util.Objects;

public final class LoanDisbursedEvent extends BankDomainEvent {

	private final LoanId loanId;
	private final AccountId checkingAccountId;
	private final BigDecimal amount;
	private final String currencyCode;

	public LoanDisbursedEvent(LoanId loanId, AccountId checkingAccountId, BigDecimal amount, String currencyCode) {
		this.loanId = Objects.requireNonNull(loanId);
		this.checkingAccountId = Objects.requireNonNull(checkingAccountId);
		this.amount = Objects.requireNonNull(amount);
		this.currencyCode = Objects.requireNonNull(currencyCode);
	}

	public LoanId loanId() {
		return loanId;
	}

	public AccountId checkingAccountId() {
		return checkingAccountId;
	}

	public BigDecimal amount() {
		return amount;
	}

	public String currencyCode() {
		return currencyCode;
	}
}
